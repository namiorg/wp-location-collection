...
Thank you for contributing to this project! This document covers the development setup, build process, and how to create a local release. For the full Git and release workflow (pull requests, tagging, and the automated release), see [docs/WORKFLOW.md](WORKFLOW.md).
...
See [docs/WORKFLOW.md](WORKFLOW.md#initial-setup) for instructions on cloning the repository.
...
- Cloning the repository and (if migrating) dropping the old `prod` remote
- Making changes via pull request and squash-merging into `main`
- Cutting a release with `bin/release.sh` or the GitHub UI

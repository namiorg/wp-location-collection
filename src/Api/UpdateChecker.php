...
use Nami\LocationData\Nami_Location_Collection;
...
class UpdateChecker {
	 ...
	 */
	protected const int|float CACHE_TIME = \DAY_IN_SECONDS; // Change this to your desired cache time.
	 ...
	 */
	protected const string REPOSITORY_BASE_URL = 'https://api.github.com/repos/namiorg/wp-location-collection/releases/latest';
	...
	public static function get_instance(): self {
		if ( ! isset( self::$instance ) ) {
			self::$instance = new self(
				Nami_Location_Collection::get_plugin_basename(),
				Nami_Location_Collection::VERSION
			);
		}
	 ...
	 */
	public function __construct( string $plugin_basename, string $plugin_current_version ) {
	 ...
	 */
	public function prepare_plugin_update_data(array $metadata_from_server ): \stdClass {
		$update_data = new \stdClass();
		$update_data->slug = $this->plugin_dirname;
		$update_data->plugin = $this->plugin_basename;
		$update_data->new_version = str_replace('v', '', $metadata_from_server['tag_name'] ?? '');
		$update_data->package = $metadata_from_server['assets'][0]['browser_download_url'] ?? '';
	 ...
	 * @return void
	 */
	public function purge(object $upgrader, array $options ): void
	{
		if (
	 ...
	 * @return void
	 */
	private function set_plugin_dirname(): void
	{
	...
	{
		$this->repository_url = self::REPOSITORY_BASE_URL;
	}
	 ...
	 */
	private function validate_metadata( $metadata ): false|array
	{
